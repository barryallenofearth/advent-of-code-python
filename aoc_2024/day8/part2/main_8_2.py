        ...
        for frequency in frequencies:
            ...
            for index in range(len(frequency_locations)):
                for comparison_index in range(len(frequency_locations)):
                    ...
                    anti_nodes.add(comparison_location)
                    ...
                    while not coordinates.is_off_grid(location_anti_node_location, min_max_coordinates[0], min_max_coordinates[1]):
                        anti_nodes.add(location_anti_node_location)
                        location_anti_node_location = Coordinates(location_anti_node_location.x - vector.x, location_anti_node_location.y - vector.y)

                    comparison_anti_node_location = Coordinates(location.x + vector.x, location.y + vector.y)
                    while not coordinates.is_off_grid(comparison_anti_node_location, min_max_coordinates[0], min_max_coordinates[1]):
                        anti_nodes.add(comparison_anti_node_location)
                        comparison_anti_node_location = Coordinates(comparison_anti_node_location.x + vector.x, comparison_anti_node_location.y + vector.y)
