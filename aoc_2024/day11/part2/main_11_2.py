def main():
    ...
    with open(file_name, encoding="utf-8") as riddle_input:
        ...
        for round in range(repetitions):
            ...
            new_sequence = []
            previous_stone_type_count = len(stones_seen)
            for stone in stones:
                stones_seen.add(stone)
                if int(stone) == 0:
            ...
            if previous_stone_type_count == len(stones_seen):
                print(f"Repetition found with {previous_stone_type_count} different stone types")
                break
