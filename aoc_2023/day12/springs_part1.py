...
for index, line in enumerate(lines):
    ...
    if line_possibilities == 0:
        regex = r"[.]*" + ''.join(["#{" + str(number) + "}[.]" + ("+" if index < len(numbers) - 1 else "*") for index, number in enumerate(numbers)])
        print(regex)
        ...
        for configuration_number in range(int(math.pow(2, question_mark_count))):
