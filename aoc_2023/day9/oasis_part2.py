...
def determine_next_number(sequences: list[list[int]]):
    ...
    for sequence_index in range(len(sequences) - 1, 0, -1):
        difference = sequences[sequence_index][0]
        last_number_in_previous_sequence = sequences[sequence_index - 1][0]
        current_last_number = last_number_in_previous_sequence - difference
        sequences[sequence_index - 1].insert(0, current_last_number)
